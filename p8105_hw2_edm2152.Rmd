...
Yes every state plus washington dc (As DC is listed)
```{r}
n_distinct(brfss_data$locationabbr)
```
...
```{r}
brfss_data$locationabbr[max(table(brfss_data$locationabbr))] #this is wrong? shows UT

max(table(brfss_data$locationabbr))
...
brfss_data %>% 
  filter(year == 2002) %>% 
  median(.data = ., excellent, na.rm = TRUE)
```
